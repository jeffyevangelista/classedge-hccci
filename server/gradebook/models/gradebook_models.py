...
class Gradebook(models.Model):
    ...
    term_id = models.ForeignKey('academicrecord.Term', on_delete=models.PROTECT, related_name='gradebooks')
    percentage = models.DecimalField(max_digits=5, decimal_places=2)
    created_at = models.DateTimeField(auto_now_add=True)
    updated_at = models.DateTimeField(auto_now=True)
    
    def __str__(self):
        return f"{self.gradebook_category_id.name} - {self.subject_offering_id} - {self.term_id}"

    class Meta:
        unique_together = ('subject_offering_id', 'term_id', 'gradebook_category_id')

    def clean(self):
        total = (
            Gradebook.objects
            .filter(subject_offering_id=self.subject_offering_id, term_id=self.term_id)
            .exclude(pk=self.pk)
